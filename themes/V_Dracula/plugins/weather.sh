...
API_KEY="8a8accbe5e5948648bd22939231501" # insert api key here
CITY="Chofu" # insert city here
...
update() {
	...
	wind_dir=$(echo $data | jq -r '.current.wind_dir' | sed 's/./&\n/g' | sort -r | uniq | tr -d '\n')
	...
	case $wind_dir in
	  ...
	  ;;
	  "WS") ICON=
	  ;; # Reversed because sort makes it in reverse order
	  "WN") ICON=
	  ;; # Reversed because sort makes it in reverse order
	esac
	
	sketchybar --set weather.info icon=$ICON label="kph: $wind_kph"
